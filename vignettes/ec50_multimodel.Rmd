...
Use `plot_EC50_curves()` to visualize raw observations and fitted dose-response curves in the same graph. The function can use the output from `estimate_EC50()` or `ec50_multimodel()`, so you do not need to repeat the formula, data, grouping columns, or model functions.
...
The curve coordinates are attached to the returned plot for additional editing or custom graphics.

```{r}
p <- plot_EC50_curves(multi_fit)
head(p$curve_data)
```
